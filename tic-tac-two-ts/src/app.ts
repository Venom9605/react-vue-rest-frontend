...
let timerElement: HTMLHeadingElement = document.createElement("h2");
timerElement.id = "timer";
container.appendChild(timerElement);

let timeRemaining: number = 5 * 60; // 5 minutes in seconds
...
function getPiecesInfo(): number {
    return game.currentPlayer === "X" ? game.playerXPieces : game.playerOPieces;
}

function renderBoard(): void {
    let existingBoard: HTMLElement | null = document.querySelector('.board');
    if (!existingBoard) {
    ...
    let newBoard: HTMLDivElement = UI.getInitialBoard(game.board, game.cellUpdateFn.bind(game), game.isInGrid.bind(game));
...
let boardContainer: HTMLDivElement = document.createElement("div");
boardContainer.classList.add("board-container");
container.appendChild(boardContainer);

let emptyBoard: HTMLDivElement = document.createElement("div");
...
let moveGridButton: HTMLButtonElement = document.createElement("button");
...
let movePieceButton: HTMLButtonElement = document.createElement("button");
...
let aiMoveButton: HTMLButtonElement = document.createElement("button");
